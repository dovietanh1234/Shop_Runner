...
namespace SHOP_RUNNER.Controllers.Gender_controller
{
    ...
    public class GenderController : ControllerBase
    {
        ...
        public GenderController(RunningShopContext runningShopContext)
        {
        ...
        [HttpGet]
        [Route("gender/get-all")]
        public IActionResult GetAll(int page = 1)
        {
            try
            {
                var cate = _context.Genders.AsQueryable();

                var result = PaginationList<Gender>.Create(cate, page, Page_size);
        ...
        [Route("gender/delete")]
        public IActionResult delete_gender(int id)
        {
            try
            {
                var cate = _context.Genders.FirstOrDefault(c => c.Id == id);

                if (cate != null)
                {
                    _context.Genders.Remove(cate);
        ...
        public IActionResult post_data(name_input_model data)
        {
            try
            {
                Gender cate_new = new Gender()
                {
                    Name = data.name
                };
                _context.Genders.Add(cate_new);
                _context.SaveChanges();
        ...
        public IActionResult alert_gender(CategoryGetAll data)
        {
            try
            {
                var cate_new = _context.Genders.FirstOrDefault(c => c.Id == data.Id);
